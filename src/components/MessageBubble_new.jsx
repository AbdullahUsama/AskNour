  ...
  if (isUser) {
    return (
      <div className="flex justify-end mb-6 message-bubble">
        <div className="max-w-[70%] bg-blue-600 text-white rounded-3xl px-4 py-3" style={{ direction: textDirection }}>
          ...
          {/* Timestamp */}
          <div 
            className="text-xs mt-2 cursor-pointer text-blue-200 text-right"
    ...
    <div className="flex justify-start mb-6 message-bubble">
      ...
      <div className="flex-1 max-w-[80%]">
        <div 
          className={`${
            message.isError
              ? 'bg-red-50 text-red-800 px-4 py-3 rounded-lg'
              : message.isWelcome
                ? 'bg-blue-50 text-gray-800 px-4 py-3 rounded-lg'
                : 'text-gray-800'
          }`}
          style={{ direction: textDirection }}
        >
          {/* Message Content */}
          <div className={`${isArabicMessage ? 'font-arabic' : ''} leading-relaxed`}>
            {messageContent.includes('\n') ? (
              <div>
                {messageContent.includes('•') || messageContent.includes('-') || /^\d+\./.test(messageContent) ? (
                  <ul className="space-y-1 list-none">
          ...
          {/* Timestamp */}
          <div 
            className="text-xs mt-2 cursor-pointer text-gray-400"
            onClick={() => setShowTimestamp(!showTimestamp)}
          >
            {showTimestamp ? formatTimestamp(message.timestamp) : '•••'}
          </div>
